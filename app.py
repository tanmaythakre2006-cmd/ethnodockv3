    ...
    for idx, row in selected_data.iterrows():
        ...
        with col_img_bot:
            ...
            if plant_photo_b64:
                ...
                st.markdown(f"""
                <div class="apple-card" style="padding:14px; text-align:center; height:100%; display:flex; flex-direction:column; justify-content:center; align-items:center;">
                    <span style="font-size:3rem;">🌿</span>
                    <div style="font-weight:600; margin-top:8px;">{row['Common Name']}</div>
                    <div style="font-size:12px; color:#86868B; font-style:italic;">{row['Botanical Name']}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        with st.expander(f"⚙️ Target Binding Cavity Controller — {row['Protein Target']} (PDB: {row['PDB ID']})", expanded=True):
            ...
            with col_btn:
                init_btn = st.button(f"📥 Initialize PDB {pdb_id}", key=f"init_{idx}", use_container_width=True)
            with col_info:
            ...
            if st.session_state.get(f'setup_done_{idx}', False):
                ...
                cx = col_c1.number_input("Center X", value=float(center[0]), format="%.2f", key=f"cx_{idx}")
                cy = col_c2.number_input("Center Y", value=float(center[1]), format="%.2f", key=f"cy_{idx}")
                cz = col_c3.number_input("Center Z", value=float(center[2]), format="%.2f", key=f"cz_{idx}")
                sx = col_s1.number_input("Size X", value=float(dims[0]), format="%.2f", key=f"sx_{idx}")
                sy = col_s2.number_input("Size Y", value=float(dims[1]), format="%.2f", key=f"sy_{idx}")
                sz = col_s3.number_input("Size Z", value=float(dims[2]), format="%.2f", key=f"sz_{idx}")
                ...
                if st.button(f"🚀 Execute Molecular Simulation for {active_compound_name}", key=f"dock_{idx}", use_container_width=False):
        ...
        if st.session_state.get(f'docking_done_{idx}', False):
            ...
            with col_sel:
                ...
                selected_mode_str = st.selectbox("Inspect Conformation Mode:", pose_options, key=f"pose_select_{idx}")
            ...
            if poses and selected_idx < len(poses):
                ...
                with col_3d:
                    ...
                    with style_c1:
                        receptor_style = style_c1.selectbox("Receptor View", ['cartoon', 'stick', 'sphere'], index=0, key=f"rec_s_{idx}")
                    with style_c2:
                        ligand_style = style_c2.selectbox("Ligand View", ['stick', 'sphere', 'cross'], index=0, key=f"lig_s_{idx}")
                    with style_c3:
                        show_surface = style_c3.checkbox("Pocket Mesh", value=False, key=f"surf_s_{idx}")

                    with open(receptor_pdbqt, 'r', encoding='utf-8') as rf:
                        receptor_str = rf.read()

                    viewer_html = inter_eng.build_3dmol_html(
                        container_id=f"viewer_macos_{idx}",
                ...
                if variants:
                    ...
                    selected_var_label = st.selectbox("Select Rational Lead Modification:", var_labels, key=f"var_sel_{idx}")
                    ...
                    with col_var_info:
                        ...
                        """, unsafe_allow_html=True)
                    with col_var_dock:
                        dock_var_btn = st.button("⚡ Dock Derivative", key=f"dock_var_{idx}", use_container_width=True)
